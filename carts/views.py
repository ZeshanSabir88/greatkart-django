...
def remove_cart(request, product_id, cart_item_id):
    ...
    # The item must be fetched by both product and its unique ID to account for variations
    cart = Cart.objects.get(cart_id=_cart_id(request))
    product = get_object_or_404(Product, id=product_id)

    try:
        # Filter by cart_item_id to ensure we are removing the correct variation item
...
def remove_cart_item(request, product_id, cart_item_id):
    ...
    # Fetch by unique cart_item_id to delete the specific variation item
    cart_item = CartItem.objects.get(product=product, cart=cart, id=cart_item_id)
    cart_item.delete()
    return redirect('cart')
